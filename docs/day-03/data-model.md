...
Pair acceptance also records its initial consumed challenge and scan receipt against the newly formed pair, returning that receipt for the pillion's own attestation. Historical SOS pairing is derived from the pair interval at validated capture time. A short SOS reconfirmation grant uses metadata in its separate command receipt; serialize it and SOS acceptance with the same actor/event advisory lock, and never mistake grant issuance for accepted SOS delivery.
