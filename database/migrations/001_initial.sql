-- Day 4 initial migration, promoted from the Day 3 reference schema.
-- Cross-row business rules still require the planned service transactions.
-- The runner owns the transaction and schema_migrations ledger.
...
REVOKE ALL ON ALL TABLES IN SCHEMA ridr FROM PUBLIC, anon, authenticated;
REVOKE ALL ON ALL SEQUENCES IN SCHEMA ridr FROM PUBLIC, anon, authenticated;

-- Only the backend can access domain rows; it cannot alter schema or migrations.
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA ridr TO ridr_api;
REVOKE ALL ON ridr.schema_migrations FROM ridr_api;
