...
test('ride HTTP routes use verified actors, strict envelopes, safe errors and correct mutation status', async (t) => {
  ...
  const store: RideStore = {
    sos: async (_account, _rideId, eventId) => ({
      id: eventId,
      rideId: ride.id,
      reporterMemberId: membership.id,
      reporterName: 'Test rider',
      deviceId: randomUUID(),
    ...
    sosList: async () => [],
    sendSos: async (_account, event, deviceId) => ({
      id: event.id,
      rideId: event.rideId,
      reporterMemberId: membership.id,
      reporterName: 'Test rider',
    ...
    reconfirmSos: async (_account, _rideId, event, _at, grantId) => ({
      accepted: null,
      grant: {
        grantId,
        eventId: event.id,
    ...
    resolveSos: async (_account, _rideId, sosId) => ({
      id: sosId,
      rideId: ride.id,
      reporterMemberId: membership.id,
      reporterName: 'Test rider',
    ...
    acknowledgeSos: async (_account, _rideId, sosId) => ({
      id: sosId,
      rideId: ride.id,
      reporterMemberId: membership.id,
      reporterName: 'Test rider',
    ...
    acknowledgeAlert: async () => ({ acknowledged: true }),
